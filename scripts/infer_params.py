#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""profile.json → 爬取参数（顶掉 Stage 3.5 的三轮 AskUserQuestion）。

skill 路径在这一步停下来问用户三次（核心参数 / 列表筛选 / 最低岗位数）。命令行路径
把「推断」交给模型、把「确认」交给用户自己看一眼输出再决定跑不跑 —— 因为脚本本身
不投递、不花钱，最坏结果是爬错一轮，而爬取命令还需要用户手动执行。
