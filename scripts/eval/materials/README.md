...
run_dir 里已经跑过 `gen_materials`、`materials/` 目录有东西了，就直接评。**五维指标里这四维（字符/招呼语/章节/客观性）永远离线本——只读文件 + 算数**。术语三分类**默认走 LLM（联网配 key，有磁盘缓存）**，`--llm-recommend`（整份点评）也是可选的联网项。只有显式 `--offline` 才会全程不触网：术语分类降级为「只读缓存，没缓存回退规则」、点评跳过；那四维无论哪种都离线。
...
为什么需要它？评估器（那五维指标）要能分出「**材料好**」和「**材料坏**」。可它怎么知道自己的尺子准不准？用**真人写得好好的材料**去测，尺子应该给高分；可真人材料不保险、还牵扯版权。于是我们**手写一份确定性、公认「干净」的材料**（stub），喂给尺子：尺子要是报幻觉/问题，说明**尺子自己坏了**，不是材料的问题。这就是「干净对照组」的本意。
...
- `3` 部分缺产物（个别岗位没生成出招呼语或简历，报告照常写，那几岗标 `missing`、五维不做判定、也不参与平均值）
...
- **LLM 综合点评**（加了 `--llm-recommend` 才有）：把五维聚合结果丢给模型，让它一段话点评。失败不致命，只少这一块。
...
scripts/eval/materials/
  ...
  evaluate_materials.py   # 唯一入口：走「准备 state → 取岗位 → 生成材料 → 五维评估 → 报告」整条流水线
  metrics.py              # 五维指标。除术语分类外的四维纯算数、不联网、不 import llm
  ...
  recommend.py            # 五维聚合 + 阈值→「改哪个文件」建议 + 可选 LLM 点评
...
- `test_eval_metrics.py` —— 五维纯函数单测，别名（`k8s`⇄`kubernetes`）这类容易误报的地方单独验；`terms_source='llm'` 直接采用分类器 terms dict、规则路径无 `mode/analysis`。
