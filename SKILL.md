...
2. **列表筛选** —— 经验、职位类型、薪资下限、公司规模。**这些枚举筛选全部用多选**（`multiSelect`），用户可勾多个档位（如经验 `经验不限`+`应届生`、职位类型 `全职`+`实习`）。**经验默认是 `经验不限`**——不要按简历里的年数去收窄它；匹配阶段会做。经验的选项**必须同时给出 `经验不限` 和 `不筛选` 两个独立项**：勾了 `经验不限` 就原样传 `--experience 经验不限`（只筛明确标注经验不限的岗位）；一个都不勾（`不筛选`）才省略该 flag（所有经验档位全要）。**多选出现互斥就重写提问**：若用户勾出矛盾组合（如经验同时勾了 `经验不限` 和 `1-3年`——既"不要求经验"又要"1-3年"，BOSS 里不存在这种岗位；或 `在校生` 与 `10年以上` 这类反着的人生阶段），不要硬传，重新发起一次 `AskUserQuestion` 让用户重选。其余筛选第一个选项是适合候选人的默认，接受即一次点击；留空一个就跳过该筛选。
