...
| Script | Purpose |
|--------|---------|
| `pipeline.py` | The driver: `parse → infer → crawl → match → deep → merge → materials → verify → render`. `--from X` runs one stage (`match` pulls `deep`+`merge`), `--to`/`--all` runs a range. `--no-images` lowers the endpoint to `verify`, not `materials`. **Never runs `apply.py`** — `test_pipeline.py` locks that. Exit codes: 0 ok, 1 precondition/total failure, 2 usage, 3 partial |
...
| `verify_no_fabrication.py` | Technology terms in the text that gets sent (`optimized_resume` + greetings) that have no basis in `resume_text.txt` / `profile.json`. Stdlib only, no model call. `--allow` whitelists a term, `--only` narrows to job indices, `--quiet` drops the context snippets, `--resume-text` overrides the baseline. **Exit 1 = it found something** (also: no baseline, or no materials at all — nothing checked ≠ clean); 3 = some materials were unreadable and therefore unchecked. Cannot detect exaggeration, only unsupported terms; ignores `optimization_suggestions` by design |
...
| `read_thin.py` | Thin views of the big JSON files so a full JD dump never enters context. `--kind jobs\|profile\|deep` take a **file** path; `--kind ranked` takes the **run directory** and joins 序号 + 公司 + 职位 + 分数 + 判定 across `qualified_jobs` / `scored_jobs` / `job_classification` / `deep_results` (the latter aligned by `rank` through `deep_candidates`). `ranked`'s `index` is the same 1-based number as `--only` / `materials_*_N` / `apply --max`, and it deliberately does **not** sort — sorting would break that correspondence. `matched < total` flags jobs absent from every matching artifact |
| `match_index.py` | The join itself (`build_match_index` → link → 分数/判定/缺失项, `build_ranked` → the table). Shared by `gen_materials.py` (which re-exports `build_match_index`) and `read_thin.py`; it lived only on the materials side until the read side needed it too. Stdlib only, so either importer stays dependency-free. Precedence is low→high: `scored_jobs` → `job_classification` → `deep_results` |
...
| `where_am_i.py` | Infer the current stage from the run directory's artifacts and print the next commands in ~1k chars. The recovery move after a context compaction — reconstructing state by re-reading the 25k-char `auto-apply.md` is what made that run expensive. Read-only, always exits 0. For `verify` it compares `verify_report.json`'s recorded mtimes against the materials, so a regenerated resume sends you back rather than inheriting an old ✅ |
| `preferences.py` | Crawl/matching parameter preset at `assets/preferences.json` (`show` / `missing` / `save` / `crawl-args` / `clear`). Path C runs `show` and branches on the exit code (1 = none), so a returning user skips the parameter questions entirely — the largest single time saving for a repeat run. One preset, `save` merges the fields you pass (`--replace` to rewrite the whole file), never expires (`show` just reports its age). `load()` drops any key outside `ALLOWED_KEYS` and any wrong-typed value: the file is hand-editable, and that whitelist is what stops `{"auto_apply": true}` from becoming an off switch for the `gate:send` confirmation. There is deliberately no field for jobs, greeting, or send |
...
| Script | Covers |
|--------|--------|
| `test_pipeline.py` | Stage orchestration. The load-bearing assertion is the last one: **reaching the end of the pipeline still never executes `apply.py`**. Also stage ranges, stop-on-failure, quick mode's apply-pool backfill, refusing to continue on an empty crawl, and that a `verify` finding stops the run *before* `render` while `verify`'s exit 3 does not |
...
| `test_where_am_i.py` | Stage inference from artifacts: monotonic progress, quick mode never asking for deep/merge, an empty pool getting its own branch, corrupt JSON, and that the printed tail never includes `apply.py --yes`. Plus `verify` staleness: a regenerated or newly added material invalidates the previous ✅, and a corrupt `verify_report.json` counts as unchecked, never as passed |
| `test_verify_no_fabrication.py` | 62 assertions, structured around the asymmetry that a missed fabrication reaches an interview while a false positive costs one glance. Beyond the obvious catch/ignore cases: `MySQL/Redis` must not become one token, the baseline is the union of `resume_text.txt` and `profile.json`, `optimization_suggestions` is out of scope, and "no baseline" / "no materials" must exit non-zero rather than report clean |
