...
一套完整的 BOSS 直聘求职自动化工具：**爬取岗位 → 解析简历 → 智能匹配 → 可视化报告 → 自动投递**。所有 LLM 分析由 Claude Code 自身模型能力完成，**零外部 API 费用**。
...
- **插件**：通常没有持久化记录，投了什么、投了什么内容，回头看找不到。
- **本 Skill**：每次运行生成完整产物 — `matching_report.html`（可视化报告）、`job_classification.json`（分类结果）、`apply_log.json`（投递日志）、定制优化简历。

### 5. 费用

- 部分"智能匹配"插件调用自己的后端 API，**按次收费或订阅制**。
- 本 Skill 所有 LLM 分析由 **Claude Code 自身模型能力**完成，零外部 API 费用。
